...
ISR(PCINT2_vect){																	//用于判断B2下降沿的isr函数，当B2的电平发生变化的时候，检测是否为下降沿，如果是，那就实现power_cont++
	uint8_t pinc4_ago = 1;
...
void ssd_display_init(){
	
	uint8_t s0_s1 = ((PINC>>5)&0x03);					//得到末两位的s0和s1，这个末两位对应了一个值，可以用来后续操作
	index_mi = s0_s1;		
	time_remaining = init_time[s0_s1];							// 直接使用 s0_s1 作为模式索引
	display_digit(time_remaining, index_mi, cc);
...
static uint8_t last_s0_s1 = 0x00;
uint8_t now_s0_s1;
